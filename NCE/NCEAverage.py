...
class NCEAverage(nn.Module):
    ...
    def forward(self, l, ab, y, idx=None):
        # print(l.size(), ab.size(), y.size())  -> torch.Size([128, 128]) torch.Size([128, 128]) torch.Size([128])
    
        y = y.cuda()

        K = int(self.params[0].item())
        T = self.params[1].item()
        Z_l = self.params[2].item()    # -1
        Z_ab = self.params[3].item()   # -1

        momentum = self.params[4].item()
        batchSize = l.size(0)  # 128
        outputSize = self.memory_l.size(0)  # 128
        inputSize = self.memory_l.size(1)   # 128
        # score computation
        if idx is None:
            idx = self.multinomial.draw(batchSize * (self.K + 1)).view(batchSize, -1)
            # print("idx.max = ", idx.max()) ->idx.max =  tensor(8188, device='cuda:0')
            # idx.select(1, 0).copy_(y.data)
            idx.select(1, 0).copy_(y.data)  # (128, K + 1)
        # sample
        # print("idx.max = ", idx.max()) -> idx.max =  tensor(8188, device='cuda:0')
        # print("y.max = ", y.max()) -> y.max =  tensor(5759, device='cuda:0')
        weight_l = torch.index_select(self.memory_l, 0, idx.view(-1)).detach()   # 
        
        weight_l = weight_l.view(batchSize, K + 1, inputSize) # (128, K + 1, 128)
        # 转换成double
        weight_l = weight_l.double()
        ab = ab.double()
        # inputSize -> (50 * 128)
        # print("weight_l.size() = ", weight_l.size(), ", ab.size() = ", ab.size())   ->  weight_l.size() =  torch.Size([128, 21, 128]) , ab.size() =  torch.Size([128, 128])
        out_ab = torch.bmm(weight_l, ab.view(batchSize, inputSize, 1))  # （128, 50 * 128, 1)
        # sample
        weight_ab = torch.index_select(self.memory_ab, 0, idx.view(-1)).detach()
        weight_ab = weight_ab.view(batchSize, K + 1, inputSize)

        weight_ab = weight_ab.double()
        l = l.double()
        out_l = torch.bmm(weight_ab, l.view(batchSize, inputSize, 1))   # （128, 50 * 128, 1)
        ...
        # # update memory
        with torch.no_grad():
            # print(self.memory_l.device, y.view(-1).device)
