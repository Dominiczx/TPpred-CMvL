...
class SupConLoss(nn.Module):
    ...
    def forward(self, features, labels=None, mask=None):
        ...
        contrast_feature = torch.cat(torch.unbind(features, dim=1), dim=0)  # (B, 2, D) -> 2 (B, D) -> (2B, D)
...
class MlcSupConLoss(nn.Module):
    """Supervised Contrastive Learning: https://arxiv.org/pdf/2004.11362.pdf.
    It also supports the unsupervised contrastive loss in SimCLR"""
    def __init__(self, temperature=0.1, contrast_mode='all',
                 base_temperature=0.1):
        super(MlcSupConLoss, self).__init__()
        self.temperature = temperature
        self.contrast_mode = contrast_mode
        self.base_temperature = base_temperature
    ...
    def forward(self, features, labels=None, mask=None, weights=None):
        ...
        logits_mask = torch.scatter(
            ...
            torch.arange(batch_size * anchor_count).view(-1, 1).to(device),
            0
        )   # (2B, 2B), 对角线为False, 其余为True
        mask = mask * logits_mask   # 点乘，将mask的对角线位置设置为False
        ...
        # compute mean of log-likelihood over positive
        mean_log_prob_pos = (weights * log_prob).sum(1) / mask.sum(1)
